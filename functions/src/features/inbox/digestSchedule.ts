...
async function deliverScheduledDigest(_docId: string, row: Record<string, unknown>) {
  ...
  const digestType = (String(row.digestType || 'caseload') || 'caseload') as DigestType;
  ...
  for (const month of months) {
    ...
    if (digestType === 'rentalAssistance') {
      await buildAndSendRentalAssistanceDigest(to, {month, recipientName});
      continue;
    }
...
export const inboxScheduleDigest = secureHandler(
    async (req, res) => {
      ...
      const digestType = body.digestType;
