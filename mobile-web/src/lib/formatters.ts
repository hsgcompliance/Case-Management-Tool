// mobile-web/src/lib/formatters.ts
// Ported from web/src/lib/formatters.ts — no changes to logic.
...
/** Smart local display: date-only for ISO10 strings, date+time for timestamps. */
export function fmtDateSmart(value: unknown): string {
  if (!value) return "";
  if (typeof value === "string") {
    const iso10 = safeISODate10(value);
    if (iso10 && String(value).trim().length <= 10) return fmtDateUS(iso10);
