...
export type EmailTemplatesFilterState = { search: string };
export type EmailTemplatesSelection = { docId: string };

function matchesSearch(doc: OrgConfigDoc, search: string) {
  const q = search.trim().toLowerCase();
  if (!q) return true;
  return `${doc.id} ${doc.label} ${doc.subject || ""}`.toLowerCase().includes(q);
}

export const EmailTemplatesTopbar: DashboardToolDefinition<EmailTemplatesFilterState, EmailTemplatesSelection>["ToolTopbar"] = ({
  value,
  onChange,
}) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      />
      <span className="text-xs text-slate-500 dark:text-slate-400">{docsByKind.email_template.length} templates</span>
    </div>
  );
};

export const EmailTemplatesSidebar: DashboardToolDefinition<EmailTemplatesFilterState, EmailTemplatesSelection>["Sidebar"] = ({
  filterState,
  selection,
  onSelect,
}) => {
  const { docsByKind } = useOrgConfigDashboard();
  const docs = docsByKind.email_template.filter((doc) => matchesSearch(doc, filterState.search));
      ...
      {docs.map((doc) => {
        ...
        return (
          ...
          >
            ...
            <span className="block truncate text-xs opacity-70">{doc.subject || doc.id}</span>
          </button>
        );
      })}
      {!docs.length ? <div className="p-3 text-xs text-slate-500">No email templates.</div> : null}
    </div>
  );
};

export const EmailTemplatesMain: DashboardToolDefinition<EmailTemplatesFilterState, EmailTemplatesSelection>["Main"] = ({
  filterState,
  selection,
  onSelect,
}) => {
  ...
  if (!selectedDoc) return <div className="py-12 text-center text-sm text-slate-400">No email templates found.</div>;
