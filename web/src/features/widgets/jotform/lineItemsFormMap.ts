...
function creditCardSpendingOptions() {
  ...
  return {
    ...
    lineItemFieldKeys: [
      ...
      ...txs.map((tx) => tx.tssCategory),
      ...txs.map((tx) => tx.wioaCategory),
      ...txs.map((tx) => tx.pathCategory),
      returnRecord.expenseType,
      returnRecord.tssCategory,
      returnRecord.wioaCategory,
      returnRecord.pathCategory,
      returnRecord.purpose,
    ],
    customerFieldKeys: [...txs.map((tx) => tx.customerName), returnRecord.customerName],
    fileFieldKeys: [
      ...txs.flatMap((tx) => [...tx.files, ...tx.additionalFiles]),
      ...returnRecord.files,
    ],
    categoryFieldKeys: [
      ...txs.map((tx) => tx.tssCategory),
