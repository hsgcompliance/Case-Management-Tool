...
export function PreviewTable({ result, isLoading, isAdvancedLoading = false, onRun, onAdvanced }: Props) {
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {result && onAdvanced ? (
          <button type="button" onClick={onAdvanced} disabled={isAdvancedLoading} className="btn btn-sm btn-ghost disabled:opacity-60">
            {isAdvancedLoading ? "Opening..." : "Advanced"}
          </button>
        ) : null}
        <button
          type="button"
      ...
      {result && (
        <div className="flex flex-wrap gap-3 text-sm">
          ...
          <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium">
            {result.projectedCount ?? result.matched.filter((item) => item.queueStatus !== "posted").length} projected
            {" / "}
            {fmtAmount(result.projectedAmount ?? result.matched.filter((item) => item.queueStatus !== "posted").reduce((sum, item) => sum + Number(item.amount || 0), 0))}
          </span>
          <span className="px-2 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-800 dark:text-indigo-300 font-medium">
            {result.postedCount ?? result.matched.filter((item) => item.queueStatus === "posted").length} posted
            {" / "}
            {fmtAmount(result.postedAmount ?? result.matched.filter((item) => item.queueStatus === "posted").reduce((sum, item) => sum + Number(item.amount || 0), 0))}
          </span>
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-xs">
                      <span className={[
                        ...
                        {item.queueStatus === "posted" ? "Posted" : "Projected"}
                      </span>
                    </td>
