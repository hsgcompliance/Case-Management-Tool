...
export const DEFAULT_TABLE_COLUMN_PART = "header";

type SortPrimitive = string | number | null | undefined;

export function useTableSort(initial: SortState = null) {
  const [sort, setSort] = React.useState<SortState>(initial);
  const onSort = React.useCallback((col: string, defaultDir: SortDir = "asc", part = DEFAULT_TABLE_COLUMN_PART) => {
    setSort((prev) =>
      prev?.col === col && (prev.part || DEFAULT_TABLE_COLUMN_PART) === part
        ? { col, dir: prev.dir === "asc" ? "desc" : "asc", part }
        : { col, dir: defaultDir, part },
    );
  }, []);
  const setSortDir = React.useCallback((col: string, dir: SortDir, part = DEFAULT_TABLE_COLUMN_PART) => {
    setSort({ col, dir, part });
  }, []);
  return { sort, onSort, setSortDir, setSort };
}
...
export function normalizeFilterValue(value: SortPrimitive) {
...
function dateValue(value: string): number | null {
  ...
  const parsed = Date.parse(raw.length === 10 && raw[4] === "-" ? `${raw}T00:00:00` : raw);
  return Number.isNaN(parsed) ? null : parsed;
}

export function compareSmartValues(a: SortPrimitive, b: SortPrimitive, dir: SortDir = "asc"): number {
  const blankA = a == null || String(a).trim() === "" || String(a).trim() === "(Blank)";
  const blankB = b == null || String(b).trim() === "" || String(b).trim() === "(Blank)";
  ...
  const an = typeof a === "number" ? a : numericValue(as);
  const bn = typeof b === "number" ? b : numericValue(bs);
  const ad = dateValue(as);
  const bd = dateValue(bs);

  let result = 0;
  if (ad != null && bd != null) result = ad - bd;
  else if (an != null && bn != null) result = an - bn;
  else result = as.localeCompare(bs, undefined, { sensitivity: "base", numeric: true });

  return dir === "asc" ? result : -result;
}

function uniqueSortedValues(values: SortPrimitive[]) {
  return Array.from(new Set(values.map(normalizeFilterValue))).sort((a, b) => compareSmartValues(a, b, "asc"));
}

function sortLabel(dir: SortDir | null) {
  if (!dir) return "Sort";
...
export function SortableHeader({
  ...
  return (
    ...
    >
      <span>{label}</span>
      <span className={active ? "text-[10px] text-slate-600" : "text-[10px] text-slate-300 group-hover:text-slate-400"}>
        {sortLabel(dir)}
      </span>
    </button>
  );
...
export function SmartRowHeader({
  label,
  col,
  sort,
  onSort,
  ...
  values: (part: string) => SortPrimitive[];
}) {
  const [open, setOpen] = React.useState(false);
  const [query, setQuery] = React.useState("");
  const rootRef = React.useRef<HTMLDivElement | null>(null);
  const availableParts = parts?.length ? parts : [{ id: DEFAULT_TABLE_COLUMN_PART, label: "Header" }];
  const [selectedPart, setSelectedPart] = React.useState(availableParts[0]?.id || DEFAULT_TABLE_COLUMN_PART);
  const activePart = filter?.part || selectedPart;
  const active = sort?.col === col && (sort.part || DEFAULT_TABLE_COLUMN_PART) === activePart;
  const dir = active ? sort.dir : null;
  ...
  const commitSelected = (nextSelected: Set<string>, part = activePart) => {
    ...
    onFilterChange(selectedValues.length === allValues.length ? undefined : { part, selectedValues });
  };
...
export const SmartFilterHeader = SmartRowHeader;

export function sortRows<T>(
  rows: T[],
  sort: SortState,
  getValue: (row: T, col: string, part?: string) => SortPrimitive,
): T[] {
  if (!sort) return rows;
  const { col, dir, part } = sort;
  return [...rows].sort((a, b) =>
    compareSmartValues(
      getValue(a, col, part || DEFAULT_TABLE_COLUMN_PART),
      getValue(b, col, part || DEFAULT_TABLE_COLUMN_PART),
      dir,
    ),
  );
}
...
export function filterRows<T>(
  ...
  return rows.filter((row) =>
    active.every(([col, filter]) => {
      ...
      const value = normalizeFilterValue(getValue(row, col, filter.part || DEFAULT_TABLE_COLUMN_PART));
