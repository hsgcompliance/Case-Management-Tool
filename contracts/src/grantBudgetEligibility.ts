...
export type GrantBudgetEligibilityReason =
  | "eligible"
  | "not-assigned-to-grant"
  | "missing-transaction-date"
  | "before-grant-start"
  | "after-grant-end"
  | "missing-line-item-assignment"
  | "unknown-line-item"
  | "outside-spending-cycle"
  | "voided"
  | "posted-queue-shadow";
...
const REASON_LABELS: Record<GrantBudgetEligibilityReason, string> = {
...
export function grantBudgetIsoDate(value: unknown): string {
 ...
 */
export function resolveGrantBudgetTransactionDate(
  row: GrantBudgetEligibilityTransaction,
): { date: string; source: string | null } {
  const fields: Array<[keyof GrantBudgetEligibilityTransaction, unknown]> = [
...
function validOverride(value: GrantBudgetEligibilityOverride | null | undefined): boolean {
  return !!value && value.approved === true && !!value.approvedBy.trim() &&
    !!grantBudgetIsoDate(value.approvedAt) && !!value.reason.trim();
}

function correctionFor(reason: GrantBudgetEligibilityReason): GrantBudgetEligibilityResult["suggestedCorrectiveWorkflow"] {
...
export function evaluateGrantBudgetEligibility(args: {
  ...
  let reason: GrantBudgetEligibilityReason = "eligible";
  ...
  const matchingCycle = eligibleForGrantTotals && resolvedDate.date
    ? cycles.find((cycle) => {
    ...
    : undefined;
  const cycleReason = eligibleForGrantTotals && cycles.length > 0 && !matchingCycle
    ? "outside-spending-cycle" as const
    : reason;
